...
CLASS ycl_aaic_rest_async_task IMPLEMENTATION.
  ...
  METHOD read.
    ...
    DATA(l_datefrom) = to_upper( i_o_request->get_form_field( i_name = 'datefrom' ) ).
    DATA(l_dateto) = to_upper( i_o_request->get_form_field( i_name = 'dateto' ) ).
